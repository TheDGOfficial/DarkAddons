...
final class GhostBlock {
    ...
    public final void onTick(@NotNull final TickEvent.ClientTickEvent event) {
        if (Config.isCreateGhostBlockWithKey() && TickEvent.Phase.END == event.phase && (GhostBlock.CREATE_GHOST_BLOCK_KEY.isPressed() || Config.isKeepHoldingToCreateMoreGhostBlocks() && GhostBlock.CREATE_GHOST_BLOCK_KEY.isKeyDown())) {
